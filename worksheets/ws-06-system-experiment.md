...
> Full NB + Full SVM + (–A) + (–B) + (–C) = **5 kondisi dasar**. Jika waktu memungkinkan, tambahkan kombinasi ganda: (–A,–B), (–A,–C), (–B,–C) = **8 kondisi**. Sesuaikan dengan *computational cost* dan tenggat waktu penelitian.

| Kondisi | Preprocessing (A) | TF-IDF Vectorizer (B) | Classifier (C) | Hasil yang Diharapkan |
| :--- | :--- | :--- | :--- | :--- |
...
| **– A (tanpa Preprocessing)** | ❌ Dinonaktifkan (teks mentah) | ✅ max_features=5.000 | ✅ NB & SVM | Penurunan F1 signifikan — membuktikan kontribusi preprocessing terhadap kualitas representasi fitur |
| **– B (tanpa TF-IDF)** | ✅ Aktif | ❌ CountVectorizer tanpa bobot IDF | ✅ NB & SVM | Penurunan F1 moderat — membuktikan kontribusi pembobotan TF-IDF dibanding frekuensi mentah (BoW) |
| **– C (baseline dummy)** | ✅ Aktif | ✅ max_features=5.000 | ❌ DummyClassifier | F1 sangat rendah — membuktikan classifier bermakna vs prediksi acak |
| **– A – B** | ❌ Dinonaktifkan (teks mentah) | ❌ CountVectorizer tanpa bobot IDF | ✅ NB & SVM | Penurunan F1 paling drastis — tanpa preprocessing dan pembobotan IDF, fitur sangat noisy dan tidak informatif |
| **– A – C** | ❌ Dinonaktifkan (teks mentah) | ✅ max_features=5.000 | ❌ DummyClassifier | F1 sangat rendah — mengkonfirmasi preprocessing + classifier adalah inti pipeline; TF-IDF saja tidak cukup |
| **– B – C** | ✅ Aktif | ❌ CountVectorizer tanpa bobot IDF | ❌ DummyClassifier | F1 sangat rendah — membuktikan TF-IDF dan Classifier keduanya esensial; preprocessing saja tidak cukup tanpa representasi fitur yang baik dan model bermakna |
